...
int main() {
    ...
    for (int i = 0; i < qtd_valores; i++) {
        int alvo = valores[i];
        int acessos_seq = 0, acessos_bin = 0;
        ...
        printf("Valor: %-8d | Acessos Seq: %-7d | Acessos Bin: %-4d\n", alvo, acessos_seq, acessos_bin);
    }
    ...
    float ganho = (float)total_acessos_seq / total_acessos_bin;
    printf("\nGanho de Desempenho: O metodo Binario foi %.2f vezes mais eficiente!\n\n", ganho);
